...
The decision logs are records of the authorization decisions made by the Cerbos PDP. These logs provide detailed information about each decision, including the input parameters, the policy evaluation process, and the final decision result (i.e., whether access was allowed or denied). Decision logs are particularly useful for auditing, troubleshooting, and understanding how Cerbos policies are being applied in real-world scenarios.



There are two views in this section. The JSON view shows the raw decision logs, whilst the decision view shows the same data in a more human-readable format. 
...
You can filter the logs by a particular PDP, principal, resource kind, action, or policy decision, as well as by time range.

=== Access logs

image:audit_logs_access.png[alt="Access logs",role="center-img"]

The access logs are records of all the authorization requests received by the Cerbos PDP. These logs provide a high-level overview of incoming requests. Access logs are primarily used for monitoring, observability, and security purposes, giving insights into how and when the authorization service is being used.
