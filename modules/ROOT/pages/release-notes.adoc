...
The xref:audit-log-collection.adoc[audit logs] page now includes an interactive timeline visualization showing decision activity over the last three months. Click individual bars to zoom into a time range, or drag to select a custom range. Bucket and tick intervals adjust dynamically based on the selected time range.
...
xref:policy-stores.adoc[Stores] can now be created from an existing store, copying all policies into a new store. This supports workflows such as promoting policies between environments or starting from a template.
