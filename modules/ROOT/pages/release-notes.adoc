...
=== Audit log exports are now rate limited

[#_audit_log_export_limit]

There is now a cap on how many xref:audit-log-collection.adoc#_exporting_logs[audit log exports] can be started from a workspace within a rolling one-hour window, so a burst of export requests cannot crowd out the rest of your organization. If you reach the cap, the new export is rejected with a message asking you to try again later, and you can start it once your earlier exports fall outside the window. Exports that are already queued or completed are unaffected and remain available to download as usual.
...
xref:audit-log-collection.adoc#_deep_linking_to_policies[Deep links from an audit log entry] to the policy behind a decision now resolve correctly. Selecting a policy name in the entry details opens the Policies tab of the deployment with the exact bundle and file from that decision already selected, so you can read the policy as it was when the decision was made rather than searching for it in the current build.
