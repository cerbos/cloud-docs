...
+
--
Service PDP::: The open source Cerbos server instances that you run in your own infrastructure. Cerbos Hub pushes pre-compiled policy bundles to service PDPs, ensuring all your data remains within your network perimeter while reducing the overhead of policy updates. See xref:decision-points.adoc[Service PDPs].

Embedded PDP::: A WebAssembly module that executes authorization logic locally within an application, without network calls to a Cerbos server. Embedded PDPs are suited for browser applications, edge functions, and other environments where latency or connectivity constraints make server-side authorization impractical. See xref:deployments-epdp-rules.adoc[Embedded PDPs].
--
