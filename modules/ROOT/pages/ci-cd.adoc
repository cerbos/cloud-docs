...
Once the policies are compiled and all the tests pass, the status of the bundle will be changed to `Generated`. The policy bundle will be automatically distributed to the Cerbos PDPs that are associated with the current label. For creation and assigment of labels see xref:cerbos-cloud:ci-cd.adoc[].
