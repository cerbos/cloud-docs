...
[IMPORTANT]
====
The WASM file from `@cerbos/embedded-server` contains only the policy evaluation engine — it does *not* contain your policies. Policies are fetched separately from Cerbos Hub at runtime by `@cerbos/embedded-client` and held in memory. You only need to update the WASM file when you upgrade the `@cerbos/embedded-server` package version. Policy updates happen automatically in the background (default: every 60 seconds) with no server restart required.
====
...
=== Server-side applications (Next.js, Express, etc.)

The embedded PDP is designed for client-side authorization, such as determining which UI elements to show or hide based on permissions. For server-side authorization — API routes, server actions, middleware — use a xref:decision-points.adoc[service PDP] with the https://www.npmjs.com/package/@cerbos/grpc[`@cerbos/grpc`] or https://www.npmjs.com/package/@cerbos/http[`@cerbos/http`] package instead.

Service PDPs run in a trusted environment and provide features not available in ePDPs, including audit logging to Cerbos Hub. Use the ePDP in the browser for presentational checks and a service PDP on the server for enforcement.
...
When listing resources — for example, showing a user all documents they can view — use `planResources` instead of calling `isAllowed` or `checkResource` for each item individually. The query planner returns a set of conditions that can be applied to filter resources efficiently, avoiding a round-trip per item.
...
The `@cerbos/embedded-server` WASM file is several megabytes. This is the policy evaluation engine and is a one-time download that can be cached by the browser. It does not grow as your policies change. For frameworks that require copying the file (such as Next.js), use the `postinstall` script approach described <<_nextjs_turbopack,above>> and add the copied WASM file to your `.gitignore` since the canonical copy is distributed via npm.
...
NOTE: Some edge runtimes have restrictions on WebAssembly APIs. For example, Cloudflare Workers does not support `WebAssembly.instantiateStreaming`. In such environments, you may need to bundle the WASM binary directly into your application or use a platform-specific loading mechanism. Consult your runtime's documentation for WebAssembly support details.
