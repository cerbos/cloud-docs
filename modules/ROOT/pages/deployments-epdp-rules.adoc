...
Embedded policy decision points (ePDPs) execute authorization logic locally using WebAssembly, without requiring network calls to a Cerbos server. Policy bundles are downloaded from Cerbos Hub and evaluated in-process by a WASM runtime.

For server-side authorization using the open source Cerbos server, see xref:decision-points.adoc[Service PDPs].
...
const cerbos = new Embedded({
  policies: {
    ruleId: "<RULE_ID>",
    scopes: ["tenant-acme", "tenant-acme.department-eng"],
  },
  wasm,
});
----

The bundle returned includes only policies applicable to the specified scopes and their ancestors.
...
The JavaScript SDK is the only supported client. The WebAssembly module is designed for JavaScript host environments (browsers, Node.js, edge runtimes).

Other platforms (mobile native, Go, Python, etc.) are not currently supported for embedded PDPs. Use xref:decision-points.adoc[service PDPs] for these environments.
