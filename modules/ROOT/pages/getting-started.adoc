...
Open the deployment created above and select the **Client credentials** tab. Click **Generate a client credential**, enter a name, and select **Read & Write** to allow policy downloads and audit log uploads. Copy both the client ID and client secret. The secret is shown only once.
...
[horizontal]
xref:decision-points.adoc[Service PDPs]:: Production deployment patterns, Kubernetes setup, reliability features, and monitoring.
xref:deployments-epdp-rules.adoc[Embedded PDPs]:: Run authorization in browsers, edge functions, and mobile apps using WebAssembly.
xref:audit-log-collection.adoc[Audit log collection]:: Stream decision logs to Cerbos Hub for analysis and compliance.
xref:policy-stores.adoc[Policy stores]:: Connect Git repositories, upload via CLI, or use the SDKs to manage policies.
