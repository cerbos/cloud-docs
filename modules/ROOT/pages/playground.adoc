...
The playground is a development and testing environment. Changes made in the playground are not automatically pushed to your policy store or git repository.

To move policies from a playground to production:
...
. Commit and push the exported policy files to your connected xref:policy-stores.adoc[policy store] repository, preserving the directory structure expected by Cerbos (e.g. `resource_policies/`, `principal_policies/`, `derived_roles/`).
