...
The `cerbosctl` CLI tool can be used to upload policy bundles to a Policy Store in Cerbos Hub. The following command uploads a policy bundle from a local file:
...
export CERBOS_STORE_ID=...
cerbosctl hub store replace-files --store-id=$CERBOS_STORE_ID .
----
